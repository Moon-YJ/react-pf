...
/*
	...
	- Community.jsx 작업흐름
		1. localStorage 활용해서 간단한 메모장 CRUD 기능 구현
		2. 컴포넌트 내부에서 글 작성, 글 출력, 글 수정, 글 삭제기능 구현
		3. pagination 기능 구현

	- 이슈사항
		1. 특정 데이터에 대한 동적 출력이 아닌, 입력과 수정에 대한 데이터를 출력해야해서 계속 초기화되는 State가 아닌 localStorage 사용
		2. 처음 컴포넌트 마운트시 localStorage에 초기값이 없어서 오류 발생
		3. 다른 사용자의 브라우저에서는 초기에 빈 화면이 출력되는 이슈 발생
		4.	특정 포스트 수정중에 다른 포스트의 수정버튼 클릭시 모든 포스트가 수정모드로 변경되는 문제
		5. 데이터가 많아지면 한 화면에 article이 무제한으로 생김
		
	- 해결
		2, 3. 마운트되자마자 localStorage에 더미 데이터를 문자화해서 저장하여 출력되도록 작업
		4. 특정 포스트가 수정모드로 전환되면 다른 포스트들의 수정버튼 비활성화
		5. 전체 데이터 개수 대비 한 페이지당 보일 포스트 개수를 계산해서 pagination기능 구현
*/
