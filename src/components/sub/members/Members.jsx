...
/*
	...
	- Members.jsx 작업흐름
		1. input 요소에 값 입력시 실시간으로 State로 해당 값을 관리하면서 인증처리 로직 구현
		2. 인증 성공시 완료 메시지 띄우고 실패시 에러문구 출력
		3. useDebounce custom hook으로 불필요하게 특정 핸들러함수가 반복 호출되는 것을 방지(--> useDebounce 작업 가이드 p.100)
	
	- 이슈사항
		1. 폼 요소가 많아서 초반에 input요소에 담길 value의 state생성했지만 state관리 어려움
		2. 사용성을 위해 전송 버튼 클릭이 아니라 실시간으로 값 입력할때마다 에러메시지를 띄워야함
		3. onChange 이벤트에 state변경함수를 연결했더니 컴포넌트가 불필요하게 많이 재랜더링됨에 따라 핸들러 함수도 계속 호출됨
	
	- 해결방안
		1. handleChange 함수를 제작해서 실제 입력하고있는 name의 폼 요소를 state객체안의 키값과 변수로 활용해서 넣어주고, input의 name값에 따른 자동 state변동되도록 처리 (ES6의 대괄호를 이용한 객체의 동적 키할당 문법 활용)
		2. onChange를 통한 state변경으로 인해 빈번한 컴포넌트의 재랜더링때문에 성능저하 문제가 있을까봐 구글링해보니 리액트는 가상돔요소가 실제 DOM요소를 통째로 만들지 않고 DOM이 변경될 데이터만 담은 객체이고, 동일한 JSX구조일때 랜더링시 동일 JSX 노드값을 재활용하기때문에 재랜더링에 대한 소비비용이 크지 않음을 알게 되었다.
		3. 컴포넌트의 재랜더링에 의해 check함수가 불필요하게 자주 호출되는 것을 막기위해 일정시간동안 연속적으로 이벤트가 발생하고 있으면(input을 계속 입력중이라면) 계속해서 핸들러함수의 호출을 지연시키는 debouncing 기능을 재활용 가능한 custom hook으로 생성하여 적용(--> useDebounce 작업 가이드 p.100)
*/
